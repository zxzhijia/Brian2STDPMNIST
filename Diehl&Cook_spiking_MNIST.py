...
if test_mode:
    scr_e = 'v = v_reset_e'    #old version scr_e = 'v = v_reset_e; timer = 0*ms' 
else:
    tc_theta = 1e7 * ms
...
neuron_groups['e'] = NeuronGroup(n_e*len(population_names), neuron_eqs_e, threshold='v>(theta - offset -52. *mV)', refractory= refrac_e, reset= scr_e,method='euler')
...
for name in population_names:
    ...
    for conn_type in recurrent_conn_names:
        ...
        connections[connName].weight= weightmatrixtemp.reshape((len(weightMatrix)*len(weightMatrix[0])))
